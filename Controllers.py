...
class Yahtzee:
    def __init__(self, master_tk):
        self.yahtzee_gui = master_tk
        self.dice = Die.Dice(5)
        self.game_board = YahtzeeViews.GameBoardView(self.yahtzee_gui, self.update_dice, empty_function, empty_function)
        self.update_dice()

    def update_dice(self):
        for die_index, die in enumerate(self.dice):
            self.game_board.update_die(die_index, die.value)
